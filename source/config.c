...
static void parse_target_field(Target* current, const char* line) {
...
static bool parse_config(const char* text, Config* cfg) {
    ...
    while (*p) {
        ...
        if (!strncmp(line, "version:", 8)) {
            ...
            char* inline_field = line + 1;
            while (*inline_field && (*inline_field == ' ' || *inline_field == '\t')) inline_field++;
            if (*inline_field) parse_target_field(current, inline_field);
        } else if (current) {
            parse_target_field(current, line);
        }
